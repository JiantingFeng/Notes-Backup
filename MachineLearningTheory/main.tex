...
\]
\begin{defn}
	Given a sequence $\{Y_k\}_{k=1}^\infty$ of random variables adapted to a filtration $\{\mcF_k\}_{k=1}^\infty$, the pair $\{(Y_k, \mcF_k)\}_{k=1}^\infty$ is a \textit{martingale} if, for all $kk\geq 1$,
\end{defn}
\printbibliography
\end{document}
